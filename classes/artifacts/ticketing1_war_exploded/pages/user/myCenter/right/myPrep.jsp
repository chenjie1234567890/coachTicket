...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单中心 <span
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort table-responsive">
            ...
            <c:forEach items="${prepList}" var="prep" varStatus="status">
                <tr class="text-c">
                    ...
                    <td>${prep.endTime}</td>
                    <td>${prep.price}</td>
                    <c:if test="${prep.back == true}">
                        <td>是</td>
                    </c:if>
                    <c:if test="${prep.back == false}">
                    ...
                    <td class="f-14 td-manage">
                        <c:if test="${prep.way == false}">
                            <a style="text-decoration:none" class="ml-5"
                               href="<%=basePath%>PayServlet?prepId=${prep.prepId}" title="付款"><i class="Hui-iconfont">&#xe71c;</i></a>
                        </c:if>

                        <c:if test="${prep.way == true && prep.back == true}">
                            <a style="text-decoration:none" class="ml-5"
                               href="<%=basePath%>UpdatePrepServlet?temp=2&&prepId=${prep.prepId}" title="改签"><i
                                    class="Hui-iconfont">&#xe6df;</i></a>
                            <a style="text-decoration:none" class="ml-5"
                               href="<%=basePath%>DeletePrepServlet?prepId=${prep.prepId}" title="退票"><i
                                    class="Hui-iconfont">&#xe6e2;</i></a>
                        </c:if>
                        <c:if test="${prep.way == true && prep.back == false}">
                            无
                        </c:if>
                    </td>
                </tr>
